...
- Installment checkout status polling removed entirely from `use-checkout-controller.ts` (was periodically calling `syncInstallmentCheckoutStatus` and could leave the launch button/banner stuck mid-poll). Pending order code/redirect URL still persist via cookies/localStorage so the "open bank page" banner works, but nothing auto-refreshes status anymore — confirmation now only happens via the bank's redirect back, not background polling.
