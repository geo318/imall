...
export async function VendorSpotlight() {
  ...
  const vendors = await getSpotlightShops(4);
          ...
          {vendors.map((vendor) => (
            ...
            >
              ...
              <div className="mt-2 inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">
                <span>{t("vendorSpotlight.products", { count: numberFormatter.format(vendor.productCount) })}</span>
                <span className="text-slate-400">•</span>
                <span className="text-slate-500">
                  {t("vendorSpotlight.sales", { count: numberFormatter.format(vendor.salesCount) })}
                </span>
              </div>
            </Link>
          ))}
