...
export async function GET(
  ...
  try {
    ...
    const response = await backendRequest(`/admin/${shopSlug}/variant-options`);
    if (!response.ok) {
      return handleBackendError(response, "Failed to fetch variant options");
    }
...
export async function POST(
  ...
  try {
    ...
    if (!response.ok) {
      return handleBackendError(response, "Failed to create variant option");
    }
    const data = await response.json();
    return NextResponse.json(data, { status: response.status });
  } catch (error) {
