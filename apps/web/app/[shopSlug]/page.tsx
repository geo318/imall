...
export default function ShopPage({ params }: { params: { shopSlug: string } }) {
  ...
  useEffect(() => {
    async function loadProducts() {
      try {
        const res = await fetch(`${env.NEXT_PUBLIC_DOMAIN}/api/shops/${shopSlug}/products`);
        if (res.ok) {
  ...
  return (
    <div className="flex flex-col min-h-screen p-4">
      <header className="text-center py-12">
        ...
        <h2 className="text-3xl text-green-600 font-semibold mb-4">You Earn It</h2>
        <p className="text-gray-600 max-w-md mx-auto">
          Sell anywhere with paid shareable links. Upload your product, set your price and share
          your link!
        </p>
      </header>
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {products.map((product) => (
              ...
              >
                ...
                {product.description && (
                  <p className="text-gray-500 mb-2 truncate">{product.description}</p>
                )}
                <Link
